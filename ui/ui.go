...
func NewProgram(cfg Config, channel chan<- string, items []list.Item) *tea.Program {
	if cfg.Logfile != "" {
	...
	return tea.NewProgram(newModel(cfg, channel, items), opts...)
}
...
func newModel(cfg Config, channel chan<- string, items []list.Item) tea.Model {
	...
	model.list.Title = "Available tasks for this project"
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "q":
			...
			log.Println("q pressed. quitting...")
			close(m.channel)
			return m, tea.Quit
		...
		case "ctrl+c":
			log.Println("ctrl+q pressed. quitting...")
			close(m.channel)
			return m, tea.Quit
