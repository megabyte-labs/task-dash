...
var sections = map[sectionKey]section{
	taskSection: {
		key:      taskSection,
		docTypes: NewDocTypeSet(TaskDoc),
		label:    "task",
	},
	bookmarkSection: {
		key:      bookmarkSection,
		docTypes: NewDocTypeSet(BookmarkDoc),
		label:    "bookmark",
	},
	docSection: {
		key:      docSection,
		docTypes: NewDocTypeSet(DocsDoc),
		label:    "doc",
	},
	newsSection: {
		key:      newsSection,
...
func New(items []Item, delegate ItemDelegate, width, height int) Model {
	...
	m.sections = append(m.sections, sections[taskSection])
	m.sections = append(m.sections, sections[bookmarkSection])
	m.sections = append(m.sections, sections[docSection])
	m.sections = append(m.sections, sections[newsSection])
...
func (m Model) getItemByType(types ...DocType) []Item {
	var agg []Item
...
func (m Model) VisibleItems() []Item {
	...
	// return m.items
	return m.getItemByType(m.currentSection().docTypes.AsSlice()...)
}
...
// GetDocType returns the current doc type.
func (m Model) GetDocType() DocType {
	return m.GetDocType()
}
...
func (m Model) countTasks(t DocType) int {
	var found int
...
func (m Model) View() string {
	...
	// Tabs
	// start tabs section
	taskCount := m.countTasks(TaskDoc)
	bookmarkCount := m.countTasks(BookmarkDoc)
	docCount := m.countTasks(DocsDoc)
	newsCount := m.countTasks(NewsDoc)
	...
	for i, v := range m.sections {
		...
		switch v.key {
		case taskSection:
			s = fmt.Sprintf("%d %s", taskCount, v.label)
		case bookmarkSection:
			s = fmt.Sprintf("%d %s", bookmarkCount, v.label)
		case docSection:
			s = fmt.Sprintf("%d %s", docCount, v.label)
		case newsSection:
			s = fmt.Sprintf("%d %s", newsCount, v.label)
		}

		if m.sectionIndex == i {
			s = m.Styles.SelectedTabStyle.Render(s)
		} else {
			s = m.Styles.TabStyle.Render(s)
		}

		tabsRender = append(tabsRender, s)
	}
	tabsWithDivider := strings.Join(tabsRender, dividerBar)
	tabsWithDivider += "\n"
	availHeight -= lipgloss.Height(tabsWithDivider)
	sections = append(sections, tabsWithDivider)
	// end tabs section
